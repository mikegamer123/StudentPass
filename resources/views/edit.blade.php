    ...
    <div class="content-wrapper">
        ...
        <div class="content-body" ><!-- Chart -->
            ...
            <div class="row match-height" style="flex-direction: column;">
                <form method="post" id="add-form" @if(isset($input["inputFile"]))@if($input["inputFile"]) enctype="multipart/form-data" @endif @endif action="/editHandler/{{$input["model"]}}/{{$id}}">
                    <div class="row">
                                    ...
                                    <div class="card-block">
                                        <div class="card-body">
                                            <fieldset class="form-group">
                                                <input type="{{$input["inputFormat"][$i]}}"  @if($input["inputFormat"][$i] == 'number') max="5" step="1" min="1" @endif name="{{$input["inputNames"][$i]}}" @if($input["inputFormat"][$i] == 'checkbox') value="1" @if($input["inputVals"][$i]) checked @endif @else value="{{$input["inputVals"][$i]}}" @endif class="@if(in_array($input["inputNames"][$i],$input["required"])) required @endif form-control">
                                            </fieldset>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="d-flex w-100" style="justify-content: space-around;"><button data-modal="#validation-modal" type="submit" class="btn add-btn btn-primary">Izmeni</button></div>
                    </div>
                </form>
            </div>
        </div>
